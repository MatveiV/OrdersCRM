...
async def get_behaviors(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    return await BehaviorCRUD.get_all(db, skip, limit)


@router.get("/{lead_id}", response_model=BehaviorResponse)
async def get_behavior(lead_id: int, db: AsyncSession = Depends(get_db)):
...
async def update_behavior(lead_id: int, behavior: BehaviorUpdate, db: AsyncSession = Depends(get_db)):
...
async def delete_behavior(lead_id: int, db: AsyncSession = Depends(get_db)):
